...
void run_one_instruction(Instruction inst, EmbeddingHolder* users, EmbeddingHolder* items, std::mutex& mtx, std::priority_queue<int, std::vector<int>, std::greater<int>>& update_queue) {
    switch(inst.order) {
        ...
        case UPDATE_EMB: {
            ...
            EmbeddingGradient* gradient_user = calc_gradient(users->get_embedding(user_idx), items->get_embedding(item_idx), label);
            EmbeddingGradient* gradient_item = calc_gradient(items->get_embedding(item_idx), users->get_embedding(user_idx), label);

			users->update_embedding(user_idx, gradient_user, 0.01);
            users->lock_list[user_idx]->unlock();   //// release user's lock

            items->update_embedding(item_idx, gradient_item, 0.001);
        ...
        case RECOMMEND: {
        	...
        	Embedding* recommendation = recommend(users->get_embedding(user_idx), item_pool);
        	users->lock_list[user_idx]->unlock();
        	for (int item_idx : item_idxes) {
            	items->lock_list[item_idx]->unlock();
           	}
...
int main(int argc, char *argv[]) {
    proj1::EmbeddingHolder* users = new proj1::EmbeddingHolder("data/q5.in");
    proj1::EmbeddingHolder* items = new proj1::EmbeddingHolder("data/q5.in");
    proj1::Instructions instructions = proj1::read_instructrions("data/q5_instruction.tsv");
    {
